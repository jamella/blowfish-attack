...
class MasterBH implements Master, MasterOverhead {
...
public MasterBH(int dictionarySize){
...
public Guess[] attack(byte[] ciphertext,byte[] knowntext) throws RemoteException {
	...
	spreadAttack(0,dictionarySize, false);
...
public Guess[] attackOverhead(byte[] ciphertext, byte[] knowntext) throws RemoteException {
	...
	spreadAttack(0,dictionarySize, true);
 ...
 */
protected void spreadAttack(long initialWordIndex,long finalWordIndex, boolean overhead) throws RemoteException {
	...
	for (ConcurrentMap.Entry<Integer, Slave> entry : slaveMap.entrySet()) {
		SlaveAttack attack;
		if(extraPartition-- > 0) {
			attack = new SlaveAttack(entry,aux,aux+slavePartition+1,this, overhead);
			attacks.add(attack);
			aux += slavePartition + 1;
		}
		else{
			attack = new SlaveAttack(entry,aux,aux+slavePartition,this, overhead);
			attacks.add(attack);
			aux += slavePartition;
		}
...
public void watchAttack() throws RemoteException {
	...
	while (i.hasNext()) {
		...
		if(attack.getLastCall() < System.currentTimeMillis() - WATCH_TIMER) {
			...
			spreadAttack(attack.getCurrentWordIndex(),attack.getFinalWordIndex()+1, false);
		}
	}
}
...
public static void main(String[] args) {
	try {
		...
		MasterBH master = new MasterBH(dictionary.size());
